...
We report two granularities of fit. \emph{Pooled per-GPU fits} use the raw (non-aggregated) samples for a given device; their 95\,\% confidence intervals are obtained by row-level bootstrap with 2000 resamples. \emph{Per-cell fits} group raw samples by $(\text{GPU}, \text{model}, \text{dtype}, \text{batch size}, \text{context window})$ and yield one $R^2$ per cell; we summarise these by their per-GPU median, mean, and standard deviation. MAPE is the standard $\text{mean}(|y-\hat{y}|/|y|)$ over the same pooled samples that produce the fit, with bootstrap CIs from the same procedure.
