...
\title{Evaluating MFU as a Portable Power Proxy for Energy-Aware LLM Training Simulation}
...
This work evaluates whether Model FLOPS Utilization (MFU), an analytical metric relating achieved throughput to peak hardware capability, can serve as a portable, software-defined predictor of GPU power for large language model (LLM) training.
Across 504 training configurations on five GPUs from NVIDIA and AMD, we show that a linear MFU-based power model fits every device ($R^2$ 0.68--0.84), including the AMD MI210 where the available utilization counter is a binary activity flag and admits no linear fit at all.
The fitted slope is hardware-specific by construction, since MFU is peak-FLOPS-normalized.
The model class itself, however, transfers across vendors and requires only a one-shot per-GPU calibration.
Under regime conditioning on dtype and batch size, within-cell MAPE drops to $\sim$1\,\%.
\end{abstract}
...
\begin{itemize}
    \item A linear MFU-power model fits every GPU we measure ($R^2$ 0.68--0.84), including AMD MI210 where GPU Utilization is binary and cannot be fit.
    \item The slope is hardware-specific (MFU is peak-FLOPS-normalized), but the model class transfers. Conditioning on dtype and batch size alone drops within-cell MAPE from $\sim$10\,\% to $\sim$1\,\%.
    \item At batch~1, MFU collapses below 1.3\,\% while power stays at $\sim$60\,\% of the batch-128 level. This marks the scope boundary of an MFU-based proxy.
\end{itemize}
\noindent Our benchmarking pipeline and data are publicly available to support reproducible energy-aware simulation studies: \url{https://github.com/dos-group/gpu_power_benchmark}. We use internal GPU telemetry (NVML on NVIDIA, ROCm on AMD) as the reference power signal throughout; Appendix~\ref{sec:power-validation} documents agreement with external wall-power measurements on the two devices where both are available.
...
MFU relates observed throughput (Tokens/s) to the theoretical compute required per token relative to the peak compute capacity of the GPU for the specific numerical precision (dtype) used.
...
\begin{equation*}
    \text{MFU} = \frac{\text{Tokens/s} \cdot \text{FLOPS}_{\text{req}}}{\text{FLOPS}_{\text{max}}},
\end{equation*}
...
\begin{itemize}
    ...
    \item \textbf{3 numerical precisions:} float32, float16, bfloat16\footnote{The Quadro RTX 5000 lacks hardware bfloat16 support; its grid uses only float32 and float16.}.
...
\begin{table}[t]
    \centering
    \caption{Per-GPU linear MFU-based power fits over all configurations. Errors are 95\,\% bootstrap CIs. ``---'' marks predictors that admit no linear fit.}
...
\begin{figure}[t]
    ...
    \caption{Predicted versus measured GPU power, one panel per accelerator. Color encodes batch size (log scale); marker shape encodes numerical precision. \emph{Top:} the per-GPU pooled linear MFU model of this subsection. Batch-1 configurations (yellow) cluster off the diagonal because the memory-bound regime breaks the single-slope fit (Section~\ref{sec:failure-modes}). \emph{Bottom:} the same data under (dtype, batch)-conditioned models (Section~\ref{Relationship Across Variables}); predictions collapse onto the diagonal across batch sizes and dtypes. The empty NVIDIA~L4 column is a placeholder for ongoing measurements.}
...
Holding numerical precision and batch size fixed, MFU explains 98\,\% of power variance on the MI210 (median $R^2 = 0.997$ across 21 cells; mean $0.980$, std $0.038$). The corresponding NVIDIA medians range from $0.89$ (RTX 4070 Ti) to $0.98$ (A100, Quadro 5000); their lower means and higher dispersion are driven by saturation cells at the largest batch sizes, where both MFU and power vary by less than 3\,\% and $R^2$ is dominated by sub-percent measurement noise (Section~\ref{Relationship Across Variables}). The pooled $R^2$ values in Table~\ref{tab:per_gpu_mfu_power} are regime-averaged mixtures rather than evidence of a noisy underlying relationship. Fig.~\ref{fig:pred_vs_meas_power} (top row) visualizes this fit: predicted-versus-measured points hug the diagonal at high batch sizes (purple) and fall off at batch~1 (yellow), foreshadowing the regime boundary studied in Section~\ref{sec:failure-modes}.

\takeaway{A linear MFU-based power model fits every device we measured, including the AMD~MI210 where GPU~Utilization is a binary activity flag and admits no linear fit. The \emph{model class}---linear, MFU-driven---transfers across vendors and precisions; the per-device \emph{slope} does not, because MFU normalizes by peak FLOPS. Supporting a new accelerator therefore needs only a one-shot per-GPU calibration sweep.}
...
Fitting a separate linear MFU-power model per (dtype, batch) cell collapses the prediction error from $\sim$10\,\% pooled across the search grid to $\sim$1\,\% within each cell, on every device. Fig.~\ref{fig:Prediction Error Figure} (bottom) shows the resulting error distributions; Fig.~\ref{fig:pred_vs_meas_power} (bottom) shows predicted versus measured power hugging the diagonal across both batch sizes and dtypes. The tail behavior of the pooled fit is informative on its own: GPU~Utilization's pooled 95th-percentile error stays below 26\,\% on every NVIDIA device, while MFU can exceed 30\,\% (worst case 57.9\,\% on the Quadro~5000), with the heavier-tail outliers clustering under reduced precision. The RTX~4070~Ti reverses this ordering: both predictors fit it worst overall, reflecting the workstation environment in which it was measured (Section~\ref{Limitations}).

\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/prediction_error.pdf}
    \caption{Absolute percentage error distributions for per-GPU linear power models based on MFU (blue) and GPU~Utilization (orange), one panel per accelerator. \emph{Top:} a single pooled per-GPU fit. \emph{Bottom:} the same predictors fit per (dtype, batch) cell --- the conditioning collapses the bulk of the distribution to within a few percent on every device. The MI210 has only an MFU box because its GPU~Utilization signal is binary (Section~\ref{Predictor Performance}); the empty NVIDIA~L4 panel is a placeholder for ongoing measurements.}
...
MFU normalizes throughput by peak FLOPS, so any workload bottlenecked on memory bandwidth rather than arithmetic will execute with low MFU even when power and GPU~Utilization remain high. Our existing search grid already brackets this boundary at its low-arithmetic-intensity end. Fig.~\ref{fig:power_vs_mfu_by_batch} makes this regime split visible: batch~1 configurations (yellow) form a near-vertical column at near-zero MFU but draw 50--75\,\% of the device's peak power, while higher batches (purple) trace the linear MFU-power trend that the model of Section~\ref{Predictor Performance} fits. Table~\ref{tab:batch1_memory_bound} quantifies the gap: MFU collapses by 35--75$\times$ across the five GPUs from batch~1 to batch~128, but power changes by only $1.6$--$2.0\times$. At batch~1, every GPU runs below 1.3\,\% MFU while drawing 50--65\,\% of its batch-128 power. The slopes of Section~\ref{Predictor Performance} therefore extrapolate downward through this regime and substantially under-predict power, with a 70--120\,W absolute error on the data-center cards.

\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/power_vs_mfu_by_batch.pdf}
    \caption{GPU power against MFU per device, with batch size on a log color scale. Batch~1 (yellow) forms a vertical column at near-zero MFU but non-trivial power, the signature of memory-bound execution; higher batches (purple) populate the diagonal trend that the linear MFU power model fits. The empty NVIDIA~L4 column is a placeholder for ongoing measurements.}
    \label{fig:power_vs_mfu_by_batch}
\end{figure*}
