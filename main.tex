...
To validate MFU as a simulation proxy, we develop a cross-architectural benchmarking framework that maps high-level model parameters to empirical power consumption. We organize the framework into the workloads we run (\S\ref{sec:workloads}), the protocol used to measure them (\S\ref{sec:execution}), and the analysis pipeline that turns the raw logs into the regressions reported in \S\ref{results} (\S\ref{sec:analysis}).

\subsection{Workloads}\label{sec:workloads}
Each measurement run drives the GPU with a causal-language-modelling \emph{training} step (forward pass, backward pass, and AdamW optimizer step), so that observed power reflects the full compute graph a training simulator would target. Inputs are a single fixed English prompt replicated to the configured batch size, tokenized with the model's own tokenizer, and padded or truncated to the requested context length. This keeps the data pipeline out of the measurement and isolates on-device compute.
...
A telemetry sample is emitted once at least 1\,s of \emph{GPU-event-timed} step time has accumulated since the previous sample, rather than 1\,s of wall-clock.
Per-step duration is measured with paired CUDA events, and the in-window GPU~Utilization average is weighted by per-step GPU time.
This decouples sample rate from host-side jitter.

We use the empirical MFU variant logged by the framework: per-step training FLOPs are approximated as $3\times$ the forward FLOPs reported by CalFLOPS~\cite{calflops}, following the standard $\sim$1:2 forward-to-backward ratio for transformer training~\cite{kaplan2020scaling}.
MFU is then $\text{tokens/s} \cdot \text{flops\_per\_token} / \text{peak\_FLOPS}$.
Peak per-device, per-precision FLOPS values are pulled from the official vendor datasheets (Table~\ref{tab:peak_flops}).
...
GPU-reported power is read through NVML on NVIDIA devices and ROCm-SMI on AMD.
The GPU-side analysis throughout the paper uses the on-board sensor values.
We use the whole-node external readings only for the validation in Appendix~\ref{sec:power-validation}.
...
\subsection{Aggregation and Analysis}\label{sec:analysis}
Per-sample logs are aggregated to per-configuration means across the four repetitions. All linear fits in \S\ref{results} are ordinary least squares of GPU-reported power on the empirical (CalFLOPS-based) MFU.

We report two granularities of fit. \emph{Pooled per-GPU fits} use the raw (non-aggregated) samples for a given device. Their 95\,\% confidence intervals are obtained by row-level bootstrap with 2000 resamples. \emph{Per-cell fits} group raw samples by $(\text{GPU}, \text{model}, \text{dtype}, \text{batch size}, \text{context window})$ and yield one $R^2$ per cell, which we summarise by their per-GPU median, mean, and standard deviation. MAPE is the standard $\text{mean}(|y-\hat{y}|/|y|)$ over the same pooled samples that produce the fit, with bootstrap CIs from the same procedure.

The AMD MI210 is excluded from any GPU~Utilization aggregate: ROCm reports utilization from the \texttt{GRBM\_COUNT} counter, which acts as a binary activity flag rather than a throughput proxy and saturates at 100\,\% under any sustained training workload, so a linear $\text{power}\sim\text{utilization}$ fit is undefined on that device.
...
A linear model relating MFU to GPU-reported power fits every device we measured, including the AMD MI210 where vendor utilization gives no usable signal at all. Table~\ref{tab:per_gpu_mfu_power} reports the per-GPU pooled fit alongside the GPU~Utilization counterpart for context (fit procedure and bootstrap CIs are described in \S\ref{sec:analysis}).

The pooled MFU fits explain 68--84\,\% of variance in GPU-reported power across NVIDIA devices, and 77\,\% on AMD MI210. Mean absolute percentage errors (MAPE) range from 7.8\,\% on the L40 to 13.7\,\% on MI210. This is sufficient precision for the ``what-if'' energy comparisons targeted by training simulators. GPU~Utilization fits are tighter than MFU on NVIDIA hardware (0.67--0.87) but undefined on MI210 (see \S\ref{sec:analysis}). MFU is therefore the only predictor of the two that yields a fittable model across all six devices.
...
\begin{figure}[t]
    ...
    \caption{MFU and GPU~Utilization versus GPU-reported power. The MI210 appears only in the MFU panel because its GPU~Utilization signal is effectively binary (Section~\ref{Predictor Performance}).}
...
Elvinger et al.~\cite{elvinger-2025} analyze GPU utilization under kernel colocation and show that coarse metrics such as achieved occupancy fail to capture resource interference, while fine-grained indicators such as instructions per cycle and pipeline utilization are more expressive. Their study targets performance interference rather than energy, but illustrates that broad vendor-defined counters can obscure the activity that actually matters.

Islam et al.~\cite{islam-2024} bridge utilization and energy more directly, showing through NVML and CUPTI traces that streaming-multiprocessor activity correlates strongly with power draw during compute phases. Similarly, AccelWattch~\cite{kandiah2021accelwattch} models GPU power from microarchitectural activity counters with high fidelity. Both depend on architecture-specific instrumentation, however, which limits portability across devices and is unavailable in high-level simulation frameworks.

At system scale, Fernandez et al.~\cite{fernandez-2024} study distributed transformer training and use MFU as a performance indicator, finding that compute efficiency degrades under scaling regimes due to communication overhead. While MFU is treated as a throughput metric, the work supports the view that compute-centric measures offer a stable abstraction of workload efficiency across configurations.
...
The limits of hardware-dependent utilization metrics are particularly evident in performance modeling and simulation. Systems such as Vidur~\cite{agrawal-2025} for inference and existing LLM training simulators and performance-modeling toolkits~\cite{simumax, zhebrak2026llmclustersim, liang-2025} model LLM execution from workload structure and parallelization strategies, without access to hardware-level utilization counters. Vendor-defined metrics such as GPU Utilization or SM activity therefore cannot feed into the modeling process. Training simulators do, however, often report MFU as a performance metric, which makes it a natural integration point for the power model proposed here.

To enable energy estimation, Özcan et al.~\cite{ozcan-2025} extend the Vidur framework with utilization-derived power models that infer energy demand from execution characteristics. However, these approaches rely on implicit utilization proxies rather than explicitly defined, portable metrics, leaving the choice of utilization abstraction underspecified.
